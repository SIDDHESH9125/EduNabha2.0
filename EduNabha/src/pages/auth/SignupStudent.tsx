...
export default function SignupStudent() {
	...
	async function handleGoogleAuth(credential: string) {
		try {
			...
			const result = await googleSignIn(credential, 'student')
	...
	async function onSubmit(e: FormEvent<HTMLFormElement>) {
		...
		try {
			...
			const res = await signup({ role: 'student', email, password, profile })
			setToken(res.token)
			navigate('/student')
		} catch (error) {
		...
		<div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
			<div className="w-full max-w-xl card">
				...
				<form className="space-y-4" onSubmit={onSubmit}>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
						<input name="class" placeholder="Class/Grade" className="border border-gray-300 rounded-md px-3 py-2 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
					...
					<input name="guardianName" placeholder="Guardian name" className="border border-gray-300 rounded-md px-3 py-2 w-full focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
					...
					<p className="text-center text-sm text-gray-600">
						...
						<Link to="/login?role=student" className="text-indigo-600 hover:text-indigo-700 font-medium">
							Sign in
						</Link>
					</p>
				</form>
